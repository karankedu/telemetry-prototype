 ...
 *
 * Simulates 5,000 concurrent Sonic Jump players each firing telemetry events.
 * Players are pre-allocated once to avoid GC pressure during sustained load.
 * Events fire in micro-batches of BATCH_SIZE using Promise.allSettled so
 * a single slow request never blocks the entire round.
 */

import { randomUUID } from 'crypto';

const SERVER_URL = process.env.SERVER_URL ?? 'http://localhost:3000/api/telemetry/events';
